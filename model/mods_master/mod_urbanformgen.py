...
class UrbanFormAbstraction(UBModule):
    ...
    def __init__(self, activesim, datalibrary, projectlog):
        ...
        self.create_parameter("nres_lanemed", BOOL, "")
        ...
        self.create_parameter("roadnet_class", BOOL, "Define classification groups for major roads")
        ...
        self.create_parameter("roadclasses_arterials", LIST, "Major class names characterising arterials")
        self.create_parameter("roadclasses_highways", LIST, "Major class names characterising arterials")
        self.roadnet_class = 0
        self.roadnet_map = ""
        self.roadnet_attref = ""
        self.roadclass_artrerials = []
        self.roadclass_highways = []
        ...
        self.create_parameter("local_map2", STRING, "Data reference ID for the locality map")
        self.create_parameter("local_attref2", STRING, "The attribute name that contains the keys for features.")
        ...
        # Agricultural Land
        self.create_parameter("agg_allocres", BOOL, "Allocate % of agricultural land to residential if population")
        self.create_parameter("agg_allocnonres", BOOL, "Allocate % of agricultural land to non-residential")
        self.create_parameter("agg_allocres_pct", DOUBLE, "% of agg land to use as residential")
        self.create_parameter("agg_allocnonres_pct", DOUBLE, "% of agg land to use as non-residential")
        self.agg_allocres = 0
        self.agg_allocnonres = 0
        self.agg_allocres_pct = 10.0
        self.agg_allocnonres_pct = 5.0
    ...
    def plan_unclassified(self, A_unc, A_park, A_ref, A_rd, Atblock):
        ...
        if self.unc_customthresh and A_unc >= (
                Atblock * self.unc_customthresh / 100.0):  # CASE 1: Treat land as its own
    ...
    def build_nonres_area(self, block_attr, Aluc, type, frontage):
        ...
        if num_floors <= self.nres_maxfloors or self.nres_nolimit_floors:
            ...
            if num_floors <= self.nres_maxfloors:
                ...
                self.notify(
                    "Site was adjusted for total employment: " + str(employeddiscrepancy) + " jobs were removed.")
