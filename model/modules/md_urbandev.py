...
class UrbanDevelopment(UBModule):
    ...
    class NeighbourhoodInfluenceFunction(object):
        def __init__(self, fid, oluc, tluc):
            ...
            """
            self.functionID = fid       # The function ID
            ...
            self.distance = []          # The distance list ()
            self.weight = []            # The function y-value, weight list()
            self.datapoints = 0
            self.ubif = ""
            self.savestate = "local"    # Local or external file, tells the program about the current function.

        def export_function_to_ubif(self, filepath, projectsave=False):
            """Exports the current data to a ubif file.
            ...
            for i in range(len(self.distance)):
                f.write(str(self.distance[i])+","+str(self.weight[i])+"\n")
            f.close()
            if projectsave:
                self.ubif = filepath
                self.savestate = "local"
            return True
        ...
        def create_from_ubif(self, filepath):
            ...
            data = []
            for lines in f:
                data.append(lines.split(",").rstrip("\n"))
            f.close()
            for i in range(len(data)):
                self.distance.append(float(data[i][0]))
                self.weight.append(float(data[i][1]))
            self.datapoints = len(self.distance)
            return True
        ...
        def return_weight_by_distance(self, dist):
            ...
            """
            if dist < self.distance[0]:
                return self.weight[0]
            elif dist > self.distance[len(self.distance)-1]:
                return self.weight[len(self.weight)-1]

            for i in range(len(self.distance)-1):
                if self.distance[i] <= dist < self.distance[i+1]:
                    y1, y2 = self.weight[i], self.weight[i+1]
                    x1, x2 = self.distance[i], self.distance[i+1]
